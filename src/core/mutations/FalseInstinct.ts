...
export class FalseInstinct implements Mutation {
  id = 'false_instinct';
  name = 'Mutation Detected';
  description = 'Your instincts are being manipulated.\nDo not trust the projection.';
  ...
  async activate(puzzle: Puzzle, scene: GameScene): Promise<void> {
    try {
      ...
      // Let playFalseInstinct handle picking one color
      await puzzle.playFalseInstinct(projections);
    } catch (e) {
       console.error(e);
    }
  }
